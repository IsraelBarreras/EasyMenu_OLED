...
EasyTable::EasyTable(Adafruit_SSD1306 *displayPointer, int width, int height)
{
...
EasyTable::EasyTable(Adafruit_SSD1306 *displayPointer, int canvasWidth, int canvasHeight, int xOrigin, int yOrigin)
{
  display = displayPointer;
...
boolean EasyTable::drawTable(byte rows, byte colums, char* labels[], int selRow, int selCol)
{
  byte actualColum = 0;
  byte actualRow = 0;
...
boolean EasyTable::drawGrill(byte rows, byte colums)
{
  byte actualColum = 0;
  byte actualRow = 0;
  int rowSpace = displayHeight/rows;
...
boolean EasyTable::drawMenu(char *items[], byte numItems, int selectItem)
{
...
boolean EasyTable::drawParameterTable(byte numItems, char* itemLabels[], double* itemValues[], int selectItem)
{
  int rowSpace = displayHeight/numItems;
